...
const Dashboard: React.FC = () => {
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {/* ===== KENNZAHLENKARTEN – jetzt einheitlich mit MetricCard ===== */}
      <div style={{ display: 'flex', gap: '16px', marginBottom: '36px', flexWrap: 'wrap' }}>
        <MetricCard title="Artikel gesamt"    value={gesamtArtikel} />
        <MetricCard title="Kritische Artikel" value={kritischeArtikel} />
        <MetricCard title="Gesamtwert Lager"  value={`CHF ${gesamtwert.toFixed(2)}`} />
        <MetricCard title="Lagerorte aktiv"   value={lagerorte} />
      </div>

      {/* ===== AKTIONSBEREICH – jetzt einheitlich mit Button-Komponenten ===== */}
      <div style={{ marginBottom: '36px' }}>
        <h2 style={{ marginBottom: '14px', fontSize: '18px', color: '#333' }}>Schnellaktionen</h2>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <PrimaryButton   label="Bericht laden"  onClick={() => {}} />
          <SecondaryButton label="Exportieren"     onClick={() => {}} />
          <SecondaryButton label="Aktualisieren"   onClick={() => {}} />
        </div>
      </div>

      {/* ===== STATUSÜBERSICHT – jetzt einheitlich mit StatusChip ===== */}
      <div>
        ...
        >
          {artikelMitHandlungsbedarf.map((a) => (
            ...
            >
              ...
              {/* StatusChip ersetzt beide inkonsistenten Varianten */}
