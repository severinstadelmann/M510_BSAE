  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        {/* War: grüner, fetter Button – jetzt einheitlicher PrimaryButton */}
        <PrimaryButton label="+ Neuer Artikel" onClick={() => {}} />
      </div>

      {/* Filter-Buttons – Pill-Stil bleibt, da es eine Navigation ist, kein Primär/Sekundär-Button */}
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', border: '1px solid #e0e0e0' }}>
          <thead>
            <tr style={{ backgroundColor: '#f5f5f5' }}>
              {['#', 'Artikelname', 'Kategorie', 'Bestand', 'Minimum', 'Preis', 'Lagerort', 'Status', 'Aktionen'].map((h) => (
                <th
                  key={h}
                  style={{
                    padding: '11px 14px',
            ...
            {gefilterteArtikel.map((a) => (
              <tr key={a.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{
                  ...
                  color: a.bestand === 0 ? '#d32f2f' : a.bestand < a.minBestand ? '#e65100' : '#222',
                }}>
                  {a.bestand}
                </td>
                <td style={{ padding: '10px 14px', color: '#888', fontSize: '13px' }}>{a.minBestand}</td>
                <td style={{ padding: '10px 14px', fontSize: '13px' }}>CHF {a.preis.toFixed(2)}</td>
                <td style={{ padding: '10px 14px', fontSize: '13px', color: '#555' }}>{a.lagerort}</td>

                {/* War: lokale StatusBadge-Funktion (eckig, andere Farben) – jetzt einheitlicher StatusChip */}
                <td style={{ padding: '10px 14px' }}>
                  <StatusChip status={a.status as Status} />
                </td>

                {/* War: zwei Inline-Buttons – jetzt SecondaryButton + DangerButton-Variante */}
                <td style={{ padding: '10px 14px' }}>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <SecondaryButton label="Bearbeiten" onClick={() => {}} />
                    <SecondaryButton label="Löschen"    onClick={() => {}} />
                  </div>
                </td>
              </tr>
            ))}
      ...
      {gefilterteArtikel.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#999', background: 'white', border: '1px solid #e0e0e0' }}>
